...
export const answerReceive = functions.https.onRequest(async (req, res) => {
    const {
      evaluation, userId, interviewId, questionId, answerId
    } = req.body;
    if(!evaluation || !userId || !interviewId || !questionId || !answerId) {
      res.status(400).send("Bad request");
    }
    const firestore = getFirestore();
    const answerRef = doc(firestore, `users/${userId}/interviews/${interviewId}/questions/${questionId}/answers/${answerId}`);
  ...
  if (!process.env.ML_API_URL) {
    ...
    try {
      const MLApiResponse = await axios.post(process.env.ML_API_URL, {
        ...
        userId, interviewId, questionId, answerId,
      } );
